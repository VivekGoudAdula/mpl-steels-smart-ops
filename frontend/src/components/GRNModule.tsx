...
import { fetchGRN } from "../services/erpApi";
...
export default function GRNModule() {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      toast.error("Failed to fetch GRN transactions from ERP");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="animate-in fade-in duration-500 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pt-2">
        <div>
          ...
          <p className="text-slate-500 font-medium">Verify materials received against purchase orders and weighbridge data from ERP.</p>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-32 space-y-4">
          ...
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">Syncing Inventory Database...</p>
        </div>
      ) : (
        <DocumentTable
